...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_link_updated_at_idx\` ON \`footer_link\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`footer_link_created_at_idx\` ON \`footer_link\` (\`created_at\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`footer_link\`;`)
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
