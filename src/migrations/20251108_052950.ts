...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`footer_link\` (
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`footer_link_id\`) REFERENCES \`footer_link\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "magazines_id", "navigation_id", "services_id", "sister_sites_id", "book_store_id", "footer_link_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "magazines_id", "navigation_id", "services_id", "sister_sites_id", "book_store_id", "footer_link_id" FROM \`payload_locked_documents_rels\`;`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_footer_link_id_idx\` ON \`payload_locked_documents_rels\` (\`footer_link_id\`);`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "magazines_id", "navigation_id", "services_id", "sister_sites_id", "book_store_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "magazines_id", "navigation_id", "services_id", "sister_sites_id", "book_store_id" FROM \`payload_locked_documents_rels\`;`)
  await db.run(sql`DROP TABLE \`payload_locked_documents_rels\`;`)
  await db.run(sql`ALTER TABLE \`__new_payload_locked_documents_rels\` RENAME TO \`payload_locked_documents_rels\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_order_idx\` ON \`payload_locked_documents_rels\` (\`order\`);`)
