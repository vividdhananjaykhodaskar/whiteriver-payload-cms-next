...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`footer_menu_links\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_menu_links_order_idx\` ON \`footer_menu_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`footer_menu_links_parent_id_idx\` ON \`footer_menu_links\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_menu_updated_at_idx\` ON \`footer_menu\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`footer_menu_created_at_idx\` ON \`footer_menu\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`footer_menu_id\` integer REFERENCES footer_menu(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_footer_menu_id_idx\` ON \`payload_locked_documents_rels\` (\`footer_menu_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`footer_menu_links\`;`)
